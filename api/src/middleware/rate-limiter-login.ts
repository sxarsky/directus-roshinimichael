...
if (env['RATE_LIMITER_LOGIN_ENABLED'] === true) {
	validateEnv(['RATE_LIMITER_LOGIN_DURATION', 'RATE_LIMITER_LOGIN_POINTS']);

	rateLimiter = createRateLimiter('RATE_LIMITER_LOGIN');
		...
		if (ip) {
			try {
				...
				throw new HitRateLimitError({
					limit: +(env['RATE_LIMITER_LOGIN_POINTS'] as string),
